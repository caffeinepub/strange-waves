...
import { streamAudio, getAudioMetadata, type AudioMetadata } from '../lib/audioApiClient';
...
export function ExternalAudioPlayer({
  ...
  if (error) {
    return (
      <div className={`rounded-lg border border-red-500 bg-red-50 p-4 text-red-800 ${className}`}>
  ...
  if (minimal) {
    return (
      <div className={`flex items-center gap-3 rounded-lg border bg-white p-3 shadow-sm ${className}`}>
