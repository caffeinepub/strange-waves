...
import type { DIP721Metadata, LSP8Metadata, BridgeConfig } from './icpBridgeAdapter';
...
export async function createNFTMirror(
  ...
  console.log(`[NFT Mirror] Creating mirror for ICP token ${icpTokenId} -> Lukso token ${luksoTokenId}`);
  ...
  console.log(`[NFT Mirror] Mirror created: ${mirrorId} (status: ${mirror.status})`);
...
export function startAutoSync(mirrorId: string, config: SyncConfig): void {
  ...
  if (!config.syncInterval) {
    console.warn('[NFT Mirror] Cannot start auto-sync: syncInterval not configured');
    return;
  }
  
  console.log(`[NFT Mirror] Starting auto-sync for ${mirrorId} (interval: ${config.syncInterval}ms)`);
...
export function listNFTMirrors(filter?: {
  ...
  if (filter) {
    ...
    if (filter.luksoContractAddress) {
      mirrors = mirrors.filter(m => m.luksoContractAddress === filter.luksoContractAddress);
    }
  }
...
export async function handleNFTTransfer(
  ...
  console.log(`[NFT Mirror] Handling transfer for ${mirrorId} on ${sourceChain}`);
 ...
 * @returns Hash string
 */
export function computeMetadataHash(metadata: DIP721Metadata | LSP8Metadata): string {
...
export function validateNFTMirror(mirror: NFTMirror): {
  ...
  if (!mirror.luksoContractAddress || mirror.luksoContractAddress.trim() === '') {
