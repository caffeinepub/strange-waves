...
export async function getAudioMetadata(
  ...
  try {
    ...
    throw new Error(`Failed to fetch metadata for file ${fileId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export function streamAudio(
  ...
  const baseHost = host || (typeof window !== 'undefined' ? window.location.origin : 'https://ic0.app');
...
export async function getAudioBlob(
  ...
  try {
    ...
    throw new Error(`Failed to fetch audio blob for file ${fileId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
